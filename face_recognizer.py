...
class Face_Recognizer:
    def __init__(self,root):
        ...
        img=Image.open("face_recognizer_left.jpg")
        img=img.resize((800,1000),Image.ANTIALIAS)
        self.photoimage=ImageTk.PhotoImage(img)
        label=Label(self.root,image=self.photoimage)
        label.place(x=0,y=0,width=790,height=850)

        img2=Image.open("face_recognizer_right.jpg")
        img2=img2.resize((1000,780),Image.ANTIALIAS)
                ...
                for(x,y,w,h) in features:
                    ...
                    if confidence>75:#which can be enhanced by using neural network
                        ...
                        with open("present_absent.csv","a") as file:
                            ans=False
                            with open("present_absent.csv","r") as line_by_line:
                                lines=str(line_by_line.readline())

                                convert_to_list=lines.split(sep=",")
                                if name in convert_to_list:
                                    ans=True
                                    break
                            if ans==False:
        ...
        b1.place(x=1120,y=690,width=200,height=50)
